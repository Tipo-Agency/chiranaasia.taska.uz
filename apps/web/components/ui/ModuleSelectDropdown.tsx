...
export const ModuleSelectDropdown: React.FC<ModuleSelectDropdownProps> = ({
  ...
  const buttonSizeClass =
    size === 'xs'
      ? 'h-8 max-w-[min(100%,220px)] sm:max-w-[280px] rounded-lg pl-2.5 pr-8 text-xs font-semibold'
      : size === 'sm'
        ? 'h-9 max-w-[min(100%,240px)] sm:max-w-[300px] rounded-lg pl-3 pr-10 text-sm font-semibold'
        : 'min-h-[44px] max-w-[min(100%,280px)] sm:max-w-[320px] rounded-xl pl-3 pr-11 py-2 text-sm font-medium';
  const chevronRight = size === 'xs' ? 'right-2' : size === 'sm' ? 'right-2.5' : 'right-3';
    ...
    <div className={`relative ${className}`} ref={ref}>
      ...
      {open && (
        <>
          ...
          />
          <div
            className={`absolute top-full mt-1.5 w-[min(100vw-2rem,20rem)] max-w-[min(100vw-2rem,20rem)] max-h-[min(70vh,22rem)] overflow-y-auto overscroll-contain bg-white dark:bg-[#1f1f1f] border border-gray-200 dark:border-[#333] rounded-xl shadow-xl py-1 z-20 ${
