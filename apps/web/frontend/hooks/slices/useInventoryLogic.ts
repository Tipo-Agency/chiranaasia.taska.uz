...
import { Warehouse, InventoryItem, StockMovement, StockMovementType, StockBalance, InventoryRevision } from '../../../types';
import { api } from '../../../backend/api';

/**
 * Пересчёт складских остатков по списку движений.
 * Вынесено за пределы хука — чистая функция без зависимостей на внешний стейт.
 */
function recalcBalances(movs: StockMovement[]): StockBalance[] {
...
export const useInventoryLogic = (showNotification: (msg: string) => void) => {
  ...
  const saveWarehouses = (next: Warehouse[]) => {
    setWarehouses(next);
    api.inventory.updateWarehouses(next).catch(console.error);
  };

  const saveItems = (next: InventoryItem[]) => {
    setItems(next);
    api.inventory.updateItems(next).catch(console.error);
  };

  const saveMovements = (next: StockMovement[]) => {
    setMovements(next);
    api.inventory.updateMovements(next).catch(console.error);
  };
  ...
  const saveRevisions = (next: InventoryRevision[]) => {
    setRevisions(next);
    api.inventory.updateRevisions(next).catch(console.error);
  };
