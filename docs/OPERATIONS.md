...
- **Деплой:** джоба `deploy` выполняется только на **push** или **workflow_dispatch** в ветку **`main`**, после успешных `web` и `api`. По SSH: `git fetch`, при грязном дереве — `stash`, затем **`git checkout origin/main -- ops/scripts/deploy.sh`** (чтобы не выполнять устаревший скрипт при расхождении `main` и `origin/main`), и **[`ops/scripts/deploy.sh`](../ops/scripts/deploy.sh)**.

Скрипт деплоя:

1. `git fetch` + `git merge origin/main --ff-only`; если ветки разошлись (например после force-push) — **`git reset --hard origin/main`** (деплой-клон = `origin/main`, без локальных коммитов на `main`)
...
**Несколько репозиториев (`tipa.taska.uz`, `chiranaasia.taska.uz`, `unique.taska.uz` и т.д.):** у **каждого** репозитория на GitHub **свой** список **Actions → Secrets** — они **не копируются** между репо автоматически.

- `SERVER_HOST` / `SERVER_USER` / `SERVER_SSH_KEY` — обычно **одинаковые**, если всё на одном VPS.
- **`SERVER_PATH` обязан быть свой** — абсолютный путь к **клону этого** репозитория на сервере (`/var/www/tipa.taska.uz`, `/var/www/chiranaasia.taska.uz`, …). Каталог **должен существовать**; иначе в логе будет `cd: … No such file or directory`.
- Без `SERVER_HOST` шаг SSH падает с **missing server host**.
- Имеет смысл задать **`NGINX_SITE_NAME`** под домен этого стека (не оставлять дефолт из `deploy.sh`, если он чужой).
- В значении **`SERVER_PATH`** не должно быть **пробела или переноса строки в конце** (в UI GitHub это легко не заметить). В workflow путь **нормализуется** (trim + убрать `\r`); если ошибка «не каталог» остаётся — смотрите в логе деплоя вывод `ls` родителя и `whoami`.
