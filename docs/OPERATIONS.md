...
На **HTTPS** (`listen 443 ssl`) для сервера `tipa.taska.uz` заданы:
...
4. пересобирает фронт (`npm ci`, `npm run build:web`) и копирует **`apps/web/dist`** в `/var/www/frontend`
...
| `BACKEND_URL` | URL API для бота (по умолчанию в скрипте `http://127.0.0.1:8003`) |
...
0 3 * * * cd /var/www/tipa.taska.uz && docker compose exec -T db pg_dump -U taska taska \
